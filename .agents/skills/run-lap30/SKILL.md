...
30分をTaskの標準完成単位かつ安全な停止周期として扱う。1-LapでPLAN、DEV、同一候補に対する独立REVIEWとリスク別QA、Main所有Gitまで完了することを標準とし、2-Lapは例外上限とする。時間制約を理由に確認工程、受け入れ条件、テスト、エラー処理、可読性を削らない。

リポジトリの`AGENTS.md`と開発契約を優先する。このスキルを厳しい契約の迂回に使わない。比例的な軽量経路が未定義なら、先に安全契約変更として承認する。

## カウント対象Lapかを分類する

製品コード、テスト、runtimeまたはbuild設定、Schema、依存、生成入力、外部観測可能な挙動のいずれかが変わるTaskをカウント対象Lapとする。このTaskには完全なPLAN、QA_PLAN、DEV、同一候補に対する独立REVIEWとリスク別QAを適用する。マージ後はtree同一性と環境依存caseだけを確認する。
...
純粋な証跡保守はカウント対象Lapにしない。専用Task ID、worktree、PLAN、QA_PLAN、DEV Agent、QA Agent、単独PRをその保守のためだけに作らず、関連製品Taskの完了処理へ可能な限り含める。Mainが目的、出典、対象path、算術、除外、影響する検査、訂正方法の短いchecklistを作り、独立Reviewer 1名が出典、算術、status遷移、cross-file metadata、Schema/parser、diff scope、secret不在を確認する。影響しない製品test suiteを繰り返さない。
...
QA_PLANの各必須条件を、実行するテスト名または検査方法と`qa_execution_mode`へ対応付ける。modeは`evidence-review | focused-rerun | live-e2e`のいずれかと理由を事前指定する。未対応行を残したままDEV readyまたはgate-readyと扱わない。計画、受け入れ条件、依存関係、preflightが揃い、先行Taskが安全な区切りへ到達した時点で、次の30分境界を待たずカウント対象Lapを開始する。
...
- 5〜20分: DEVとテスト証跡作成、Reviewerは利用可能な差分を読み取り専用で先行確認
- 20〜25分: 同一候補に対する独立REVIEWとリスク別QAを並行実行し、必要な修正を限定
- 25〜30分: Mainが再実行または`qa_carry_forward`を判断し、Main所有Git、tree/環境確認、記録
...
純粋な証跡保守のために架空のTask ID、`lap_id`、`lap_started`を作らない。関連製品Taskがまだ開いている場合は、そのTaskの`git`または完了処理の証拠として記録して`lap_completed`前に閉じる。関連Taskが完了済みなら、リポジトリで承認された軽量checklistとcommit証拠を使い、Schemaに存在しない保守eventをJSONLへ無理に押し込まない。公開済みLapを再開、改番、またはLap 2化しない。
...
カウント対象の製品Taskでは次の依存関係を守る。

1. `plan`
2. `qa_plan`
3. `dev`
4. 同一candidateに対する`review`と`qa`（並行可）
5. `git`

DEV、Reviewer、QAを別担当にする。DEVはcase ID、`candidate_commit`、`candidate_tree`、command/test、環境またはfixture、cache条件、exit status、artifact digest、未実施理由を候補証跡として返す。ReviewerとQAは同じ候補を、互いのPASSを開始条件にせず評価する。Lap30 Schema上の`review`と`qa`のstage記録は実時間どおり重複してよく、event sequenceだけを単調増加させる。

Reviewer/QAが軽微と判断した指摘は、担当AgentがTask worktreeで直接修正・ステージ・コミットできる。Task branchへの取り込み後は解消済みとしてPASSにでき、DEV差し戻し、再REVIEW、再QA、`qa_carry_forward`を要求しない。挙動、要件、安全境界を変えると担当Agentが判断した場合だけ通常経路へ戻す。Mainだけが`main`へのmerge/pushを行う。

QAはTASK-first QA_PLANに従って次を行う。

- `evidence-review`: DEVの候補証跡について、受け入れ対応、test弱体化、negative case、完全性を独立監査する。無差別な再実行は要求しない。
...
最初の2〜3タスクだけを詳細化し、その後に`remeasurement`区間を置く。再計測の結果反映は可能な限り直前の製品Taskの25〜30分にあるMain所有Gitと完了処理へ含め、既知の計測値を同期するためだけの独立TaskやPRにしない。
