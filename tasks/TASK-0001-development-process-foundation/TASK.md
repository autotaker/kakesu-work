...
- [ ] 開発プロセス、Agent責務、Task管理、Git、レビュー、QAの正本文書がある。
- [ ] 対象言語とSchema・文書のコーディングガイドラインがある。
- [ ] `agent-harness-work`が隣接する独立Gitリポジトリとして初期化され、`main`一本で運用できる。
- [ ] `TASK.md`、`PLAN.md`、`REVIEW_RESULT.md`、`QA_PLAN.md`、`QA_RESULT.md`、`HANDOVER.md`をTaskごとに生成できる。
- [ ] バックログは7状態だけを使い、Epic進捗を完了ポイントとフェーズ分布で表示できる。
- [ ] 見積もりは実装コード、Schema、設定の予定ファイル数と行数から規則ベースで算出される。
- [ ] フェーズゲート、証跡、依存関係、Wiki索引を機械検査できる。
- [ ] QA FAILを実装、QA計画、要件、環境、回帰へ分類し、main Agentが差し戻し先を決める規約がある。
- [ ] Wiki Agentが許可範囲だけを更新し、HANDOVER digestによる冪等性を保ち、検査後に直接コミットできる。
- [ ] `make check`と運用リポジトリ検査がPASSする。
...
- [ ] 受け入れ条件を満たしている。
- [ ] 必要な実装、テスト、文書が完成している。
- [ ] bootstrap代替レビューと`make check`がPASSしている。
- [ ] マージ相当のmain上QAが完了している。
- [ ] HANDOVERがWiki Agentにingestされている。
