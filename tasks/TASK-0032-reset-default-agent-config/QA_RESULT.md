...
- `main` / merge tree: 未マージ / 未設定
- `merge_tree`はマージ後にMainが記録し、案 QAでは未設定とする: 確認済み
...
| QA-006 | `live-e2e` | merge後のmerge tree | `pending` | 製品merge後のMain所有`make work-config-sync`、product/work mapping照合、Codex reload/restart、runtime default 6/1・7 role/fallback観測、rollback/cleanupを実施予定。 | merge前のため実行時点未到来。環境不足によるblockedではない |
...
- 結論: `pending`（Main判断待ち。QA判定はcandidate QA-001〜QA-005 `pass`）
- 差し戻し先: なし
- revert / バグ化: なし
- 判断理由: QA-001〜QA-005は同一candidateで全てPASSし、実装不具合、requirement gap、qa_plan defect、regression、environment issueの根拠なし。QA-006はmerge後実施期日未到来の`pending`でありblockedではない。

## 未実施項目

- QA-006のみ`pending`。merge後のMain所有`live-e2e`であり、candidate QAの未実施/blockedには分類しない。
...
`pass`。candidate-bound QA-001〜QA-005は全てPASSした。QA-006はmerge後`live-e2e`として`pending`であり、Task全体のmerge後確認完了を意味しない。
