...
この計画は TASK の `planning input packet` のみを入力とする。各観測は、DEV が固定する同一の `candidate_commit` と `candidate_tree` に束縛し、HANDOVER のケース別コマンド、fixture/environment、cache、exit、digest、未実施理由を独立に突合する。候補と tree、移行元 REF-2、または PR 管理対象 path の digest が一致しない場合は、どの `evidence-review` も PASS にしない。
...
- GitHub の actor 再認証、repository の auto-merge、ruleset と実際の required check 名は planning 時点で pending である。ready 化後の値が観測範囲を変えるなら、Main は dependency-ready reconciliation と QA_PLAN 再承認なしに DEV/QA を進めない。
...
| QA-003 | AC-3 | 作成済み Task worktree の sparse list と filesystem を確認して main 管理証跡/運用 index 非 checkout を検証する。main root への明示 routing、action ごとの allowlist/共有 path、lock 競合、non-fast-forward race（2回上限）を fixture で独立再実行する。 | `focused-rerun` / Git lock と remote race は隔離 bare remote の決定的競合注入で受理真実を再現できる。 | sparse-checkout 設定・列挙、main root への書込先、action別 staged path、lock owner/log、各 retry の recheck/exit、remote ref と digest。 | 除外 path が checkout、許可以外が stage/commit、共有 path を別 action が更新、lock bypass、競合で自動解決/3回目 retry は FAIL。 |
...
| QA-005 | AC-5 | 同一 candidate commit/tree と PR 管理対象 path digest から REVIEW/QA 証跡を独立監査する。両 PASS の後、承認済み GitHub test repository で `task-pr` を実行し ready PR、merge-commit auto-merge を観測し、main 直管理 path を PR に混入させた negative を実行する。 | `live-e2e` / PR 作成、actor 権限、auto-merge は実 GitHub auth と repository 設定に依存する。 | candidate/tree と path digest、両 result の開始独立性、PR URL/number、ready 状態、auto-merge method/status、negative PR 拒否、cleanup。 | pending/無効 auth、candidate/digest 不一致、片方 PASS 前の PR、direct path 混入許容、merge method 不明は blocked/FAIL。 |
...
- DEV は candidate ごとに `candidate_commit`、`candidate_tree`、PR 管理対象 path digest と、全ケースの command/test、fixture/environment、cache、exit、artifact digest、未実施理由を HANDOVER へ固定する。
- QA は同一 candidate から REVIEW と並行・独立に開始する。修正で candidate が変われば、Main が再束縛し、影響ケースを再実行する（本 Task の security、auth、workflow、schema、concurrency、state 変更には carry-forward を適用しない）。
