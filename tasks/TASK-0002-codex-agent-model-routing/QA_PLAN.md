...
| QA-004 | main→explorer、main→role→explorer、深さ/thread と bounded 調査 | routing fixture で root→explorer（depth 1）と root→role→explorer（depth 2）を dry-run し、depth 3、explorer→child、同時 3 thread、質問 0/2 件の入力を試す。 | 二つの許可 chain は PASS。`max_depth=2`、`max_threads=2` を超える入力、explorer の child/fan-out、単一でない bounded question は起動前 FAIL。通常委譲は既定 off。 | canonical config assertion、各 dry-run JSON の depth/thread/question count、拒否 code、`commit:null`。 |
| QA-005 | explorer の read-only、簡潔な結果、再委譲禁止 | explorer fixture に編集、stage/commit、scope 拡大、spawn を試みる command を与え、別 fixture に一問への短い根拠要約と file reference だけを返す command を与える。 | explorer は `sandbox_mode=read-only`、write scope なしで、変更・Git write・spawn を拒否する。許可結果は一問に対する concise summary と file reference のみで、raw log/secret を含まない。 | sandbox/allowed-paths assertion、write/spawn 拒否結果、redacted child result JSON。 |
