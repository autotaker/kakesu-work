...
- work側adapterの生成元、生成を実行する既存initialization/governance command、およびadapter更新commitをmain Agentがいつ作成するかは、実装開始前にproduct repositoryの既存flowと照合して確定する。未文書のCodex TOML include機構やglobal設定へ依存しない。
- `agents.max_threads = 2` がroot-direct explorerとroot→role→explorerの各許可経路で意図どおりに同時実行数を制約できるかは、実装時にCodex project-scoped設定とlauncherの統合試験で確認する。設定だけで表現できない制約はlauncher validationでfail closedにする。
