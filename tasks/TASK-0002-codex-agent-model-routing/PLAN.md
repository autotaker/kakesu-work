...
- Development Task と Task Delivery に従い、PLAN承認、実装前QA計画、独立review、mainだけの`--no-ff` mergeを変更しない。
- Work Repository Boundary と DECISION-0002 に従い、製品コードとcanonical設定は`agent-harness`、Task証跡は`agent-harness-work`に置く。共通lockはwork repositoryへの書込み全期間を直列化する。
- DECISION-0001 の役割分離を固定routingと限定explorerで強化する。QA FAILは QA FAIL Attribution に従い自動的にDEV責任としない。
- Wiki Ingestion は完了後だけ適用する。本TaskでWiki本文・Schemaは変更しない。

## 設計

### 決定した構成

製品repositoryの `.codex/config.toml` と `.codex/agents/*.toml` を唯一のcanonical custom-agent configurationとする。`.codex/config.toml` は `agents.max_depth = 2` と保守的な同時thread上限 `1` を明示し、global config、未文書include key、未文書model aliasを参照しない。role contractは次の実在するproduct artifactで明示する。

| artifact | 固定contract |
|---|---|
| `../agent-harness/.codex/config.toml` | project settings、`agents.max_depth = 2`、thread limit 1、明示role registry |
...
実装/configuration/build integrationは15ファイル、1,086行。`ceil(15/3)=5`、`ceil(1086/200)=6`、最小scale値は5のため、`estimate_points: 6` とする。
