...
root Taskの責任者は人間であり、HarnessがTask/owner/workspaceを確定する。CLIがTask recordを直接書く、またはAgentを先にbusyにする方式は、失敗時に孤立ownerやworkspaceを残す。依存するTask-0007のControl transactionとTask-0009のP0 workspace adapterを組み合わせる必要がある。
